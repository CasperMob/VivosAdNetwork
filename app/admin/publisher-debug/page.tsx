                ...
                <div className="space-y-2">
                  ...
                  >
                    <code className="text-sm font-mono break-all flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-blue-400">{url.protocol}//</span>
                      <span className="text-green-400">{url.host}</span>
                      <span className="text-yellow-400">{pathname}</span>
                      {searchParams.length > 0 && (
                        <>
                          <span className="text-gray-500">?</span>
                          {searchParams.map(([key, value], idx) => (
                            <span key={idx}>
              ...
              <div className="bg-[#1A1A2E]/80 backdrop-blur-xl border border-blue-500/30 rounded-2xl shadow-2xl p-6 mb-6">
                ...
                <div className="space-y-2">
                  ...
                  <div 
                    ...
                    className="bg-[#0F0C29]/80 border border-gray-700 rounded-lg p-4 cursor-pointer hover:border-blue-500/50 transition-all group relative"
                  >
                    <code className="text-sm font-mono break-all text-blue-400">
                      {apiEndpoint}
                    </code>
                    <div className="absolute top-2 right-2 text-xs text-gray-500 group-hover:text-gray-400 transition-colors">
                      Click to copy
    ...
    <div>
      ...
      <a href={ad.target_url} target="_blank">
        Visit Ad
                ...
                <div>
                  <h3 className="text-sm font-semibold text-gray-300 mb-2">3. Or use the ChatbotAd component</h3>
                  <div 
                    onClick={() => {
                      const code = `import { ChatbotAd } from '@vivos-ad-network/sdk'
...
function MyChatbot() {
  return (
    ...
    />
                      ...
                      copySnippet(code, 'chatbotAd')
                    }}
                    className="bg-[#0F0C29]/80 border border-gray-700 rounded-lg p-4 overflow-x-auto cursor-pointer hover:border-green-500/50 transition-all group relative"
                  >
                    <pre className="text-xs font-mono">
                      <code className="text-gray-300">
                        <span className="text-purple-400">import</span> <span className="text-yellow-400">{'{'}</span> <span className="text-blue-400">ChatbotAd</span> <span className="text-yellow-400">{'}'}</span> <span className="text-purple-400">from</span> <span className="text-green-400">&apos;@vivos-ad-network/sdk&apos;</span>{'\n\n'}
                        <span className="text-purple-400">function</span> <span className="text-blue-400">MyChatbot</span><span className="text-pink-400">()</span> <span className="text-yellow-400">{'{'}</span>{'\n'}
                        {'  '}<span className="text-purple-400">return</span> <span className="text-pink-400">(</span>{'\n'}
                        {'    '}<span className="text-yellow-400">&lt;</span><span className="text-blue-400">ChatbotAd</span>{'\n'}
                        {'      '}<span className="text-gray-300">publisherId=</span><span className="text-green-400">&quot;{'{'}publisherId{'}'}&quot;</span>{'\n'}
                        {'      '}<span className="text-gray-300">keyword=</span><span className="text-green-400">&quot;{'{'}keyword{'}'}&quot;</span>{'\n'}
                        {'    '}<span className="text-yellow-400">/&gt;</span>{'\n'}
                        {'  '}<span className="text-pink-400">)</span>{'\n'}
                        <span className="text-yellow-400">{'}'}</span>
                      </code>
                    </pre>
                    {copiedSnippet === 'chatbotAd' ? (
                      <div className="absolute top-2 right-2 text-green-400 text-xs font-semibold flex items-center gap-1">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                        </svg>
